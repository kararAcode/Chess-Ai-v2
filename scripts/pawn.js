...
class Pawn extends Piece {
    ...
    getPossibleMoves() {
        ...
        if (this.board[forwardOne][this.y] == null && !this.isOutside(forwardOne, this.y)) {
            moves.push({x: forwardOne, y: this.y});
        }

        // Move two squares forward on the first turn if it's not occupied and within board limits
        if (this.board[forwardTwo][this.y] == null && this.firstTurn && !this.isOutside(forwardTwo, this.y)) {
